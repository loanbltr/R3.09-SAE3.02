    ...
    def acceptClient(self, socketMaster):
        while True:
            try:
                ...
                threadReceive = threading.Thread(target=self.receiveClient, args=(conn, address))
                threadReceive.start()
            except Exception as e:
                print(f"Error accepting connection: {e}")

    def redirectClientSlave(self, data):
        for i in range(len(self.listSlave)):
            #if int(self.slaveCpuSocket[i]["cpu"]) < 50:
            try:
                self.listSlave[i]["socket"].send(data)
                print(f"Data sent to slave {self.listSlave[i]['ip']}:{self.listSlave[i]['port']} --> {data}")
            except Exception as e:
                print(f"Error sending data to slave {self.listSlave[i]['ip']}:{self.listSlave[i]['port']}: {e}")
                continue
            break

    def receiveSlave(self, socket):
        while True:
            try:
    ...
    def receiveClient(self, conn, address):
        try:
            while True:
                ...
                self.redirectClientSlave(data)
        except Exception as e:
            print(f"Error receiving data: {e}")

    def receiveCPU(self, socket):
        #cpuTmp = 0
        #self.listSlaveCpuSocket.append({"socket": socket, "cpu": cpuTmp})
        while True:
            try:
                data = socket.recv(1024).decode()
                #self.listSlaveCpuSocket[-1]["cpu"] = float(data)
            except Exception as e:
                print(f"Error receiving data: {e}")
                return None
            ...
            with open(self.configSlave, 'r') as file:
                for line in file:
                    ...
                    try:
                        ...
                        threadReceiveSlave = threading.Thread(target=self.receiveSlave, args=(socketMaster,))
                        threadReceiveSlave.start()
                    except Exception as e:
                        print(f"Error connecting to slave {line[0]}:{line[1]}: {e}")
                    try:
                        socketMastercpu = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                        socketMastercpu.connect((line[0], 11111))
                        self.listSlaveCpu.append({"socket": socketMastercpu, "ip": line[0], "port": 11111})
                        print(f"Connection established with slave {line[0]}:{11111}")
                        threadReceiveCPU = threading.Thread(target=self.receiveCPU, args=(socketMastercpu,))
                        threadReceiveCPU.start()
                    except Exception as e:
                        print(f"Error connecting to slave {line[0]}:{11111}: {e}")
        except Exception as e:
